    ...
    def LatestInputRequest():
        ...
        if not noDB:
            ...
            for c in clientNameList:
                ...
                print(str(latestInputFlo + 1.0) + " " + str(float(c.get("latest_input"))/1000000.0) + " " + str(latestInputFlo - 1.0))
                print((latestInputFlo + 1.0) <= float(c.get("latest_input"))/1000000.0 <= (latestInputFlo - 1.0))

                if (latestInputFlo + 1.0) <= float(c.get("latest_input"))/1000000.0 <= (latestInputFlo - 1.0):
                    ...
                    def DatabaseGetLatestInputTableValueToDictMod1(_tableName, *_colName):
                        DatabaseGetLatestInputTableValueToDict(dbA, dbP, clientNameDict,
                            float(c.get("latest_input")), _tableName, tableNameList, *_colName)
